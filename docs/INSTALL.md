...
docker run --rm -it -v "$PWD/state:/state" \
  ghcr.io/pusakaai/db-wrapper:0.1.4 --env-file /state/.env test-db --init
```
...
docker run --rm --env-file .env -v "$PWD/state:/state" \
  ghcr.io/pusakaai/db-wrapper:0.1.4 enroll
```
...
sudo ./venv/bin/pip install --upgrade "qlar-db-wrapper[postgresql] @ \
  https://github.com/pusakaai/db-wrapper/releases/download/v0.1.4/qlar_db_wrapper-0.1.4-py3-none-any.whl"
sudo ./venv/bin/qlar-db-wrapper version     # should print 0.1.4
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: wrapper
          image: ghcr.io/pusakaai/db-wrapper:0.1.4
          args: ["run"]
          envFrom:
